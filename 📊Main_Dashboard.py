...
st.markdown(
    """
    <div style="background-color: #fcb4fe; padding: 20px; border-radius: 12px; border: 1px solid #000000; line-height: 1.7; font-size: 16px; margin-top:20px; margin-bottom:20px;">
...
fig1 = px.pie(
    ...
    values="SWAP_VOLUME",    
    names="SOURCE_CHAIN",    
    title="Stablecoin Swap Volume By Source Chain ($USD)"
)
...
fig2 = px.pie(
    ...
    values="SWAP_COUNT",     
    names="SOURCE_CHAIN",    
    title="Stablecoin Swap Count By Source Chain"
)
...
fig1 = px.pie(
    ...
    title="Stablecoin Swap Volume By Destination Chain ($USD)"
)
fig1.update_traces(textinfo="percent+label", textposition="inside", automargin=True)
...
fig2 = px.pie(
    ...
    values="SWAP_COUNT",     
    names="DESTINATION_CHAIN",    
    title="Stablecoin Swap Count By Destination Chain"
)
fig2.update_traces(textinfo="percent+label", textposition="inside", automargin=True)
...
# --- Row 15 ----------------------------------------------------------------------------------------------------------
@st.cache_data
def load_stablecoin_avg_volume_data(timeframe, start_date, end_date):
    start_str = pd.to_datetime(start_date).strftime("%Y-%m-%d")
    end_str = pd.to_datetime(end_date).strftime("%Y-%m-%d")
...
# --- Load data ----------------------------------------------
stablecoin_avg_volume_data = load_stablecoin_avg_volume_data(timeframe, start_date, end_date)
# ------------------------------------------------------------
